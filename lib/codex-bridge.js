...
// Allowed characters for job IDs: must start with an alphanumeric character,
// then alphanumeric with hyphens/underscores/dots. Leading dash is disallowed
// to prevent values like "--flag" being interpreted as CLI flags by the companion.
const TASK_ID_RE = /^[A-Za-z0-9][A-Za-z0-9._-]{0,127}$/;

// Loose git-ref allowlist: letters, digits, and the small set of chars git
// accepts in ref names. Prevents values like "--upload-pack=evil" or values
// with shell metacharacters from being misinterpreted by the companion.
const GIT_REF_RE = /^[A-Za-z0-9_./:@^~-]{1,256}$/;

// Valid model names: alphanumeric plus dots, hyphens, underscores (e.g. gpt-5.3-codex).
...
function runCompanionSync(subcommand, extraArgs, repoPath, timeoutMs) {
  return new Promise((resolve, reject) => {
    ...
    proc.on('error', err => { clearTimeout(killer); if (sigkillTimer) clearTimeout(sigkillTimer); reject(err); });
  });
}
...
export async function codexReview(args) {
  ...
  const timeoutMs = resolveTimeout(args.timeout_ms);

  if (args.background) {
    const { pid } = runCompanionBackground('review', passthrough, repoPath);
    return formatBackgroundLaunch('review', pid, repoPath);
  }
  return await runCompanionSync('review', ['--wait', ...passthrough], repoPath, timeoutMs);
}
...
export async function codexAdversarialReview(args) {
  ...
  return await runCompanionSync('adversarial-review', ['--wait', ...passthrough], repoPath, timeoutMs);
}
...
export async function codexRescue(args) {
  ...
  if (args.model) {
    ...
    passthrough.push('--model', model);
  }
  if (args.effort) {
    const effort = String(args.effort);
    if (!VALID_EFFORTS.has(effort)) {
      throw new Error(`Invalid effort value: "${effort}". Must be one of: ${[...VALID_EFFORTS].join(', ')}`);
    }
    passthrough.push('--effort', effort);
  }
  ...
  return await runCompanionSync('task', passthrough, repoPath, timeoutMs);
}
...
function validateTaskId(taskId) {
  ...
  if (!TASK_ID_RE.test(s)) {
    throw new Error(
      `Invalid task_id "${s}". Expected 1–128 characters, alphanumeric with hyphens/underscores/dots only.`
    );
  }
}

function resolveTimeout(rawMs) {
  if (rawMs === undefined || rawMs === null) return DEFAULT_TIMEOUT_MS;
  const n = Number(rawMs);
  if (!Number.isFinite(n) || n <= 0) {
    throw new Error(`timeout_ms must be a positive number, got: ${rawMs}`);
  }
...
function buildPassthroughArgs(args, allowedKeys) {
  const out = [];
  if (allowedKeys.includes('base') && args.base) {
    const base = String(args.base);
    if (!GIT_REF_RE.test(base)) {
      throw new Error(`Invalid base ref "${base}". Use a standard git ref (branch name, SHA, etc.).`);
    }
    out.push('--base', base);
  }
